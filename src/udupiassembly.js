...
const AssemblyListUdupi = () => {
  const classes = useStyles();
  const navigate = useNavigate()

  const submit = (id) => {
    // console.log(id)
    navigate(`/population-graph/udupi/${id}`)
  }
  let districtData;

  districtData = udupidata;

  const column1 = districtData.assembly_constituencies.slice(0, 7);
  ...
  return (
    ...
    <Toolbar style={{ display: "flex", alignItems: "center" }}>
        ...
        <Typography
          ...
          <Avatar sx={{ bgcolor: "green" }}>UD</Avatar>
        </Toolbar>
...
export default AssemblyListUdupi;
