...
import Bengaluruurbandata from "../src/topojsons/Districts/DistrictsData/Bengaluruurbandata.json";
...
function BangaloreAssembly() {
  const classes = useStyles();
  const navigate = useNavigate();
  const submit = (id) => {
    navigate(`/population-graph/Bengaluru/${id}`);
  };
  ...
  // });
  let districtData;
  districtData = Bengaluruurbandata;
  const column1 = districtData.assembly_constituencies.slice(0, 24);
...
export default BangaloreAssembly;
